...
test_that("Making sure only valid methods", {
  expect_error(alXvalidate(train, test, method = "alWRONG"), 
               "Error : `%in%`(x = method, table = c(\"rf\", \"xgb\", \"alkk\")) is not TRUE",
               fixed = TRUE)
})

testXval.RF <- alXvalidate(k = 2, m = 20, train[1:100,], NT = 1, method = "rf")
testXval.XG <- alXvalidate(k = 2, m = 20, train[1:100,], NR = 5, method = "xgb")
